...
# elf input separated by new line
# Find the Elf carrying the most Calories. How many total Calories is that Elf carrying?

with open('day1_calorie_counting_input.txt', 'r', encoding='utf-8') as elf_input:
    elf_caravan_shipping_log = elf_input.readlines()

# first elf should = 55113
most_elf_calories = 0
current_elf_calories = 0
for entry in elf_caravan_shipping_log:
    if entry != "\n":
        current_elf_calories += int(entry.strip())
    elif current_elf_calories > most_elf_calories:
        most_elf_calories = current_elf_calories 
        current_elf_calories = 0
    else:
        current_elf_calories = 0

print(most_elf_calories)
